...
python -m gridsteer.step1.optimize_phi "/qfs/projects/bioprep/data/automation/new_grid_center_db.2/" 99 --verbose
"""
...
class AnalyzerState:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'min_width_found': self.min_width_found,
...
class FrameProcessor:
    ...
    def process_frames(self):
        ...
        try:
            for frame_number in frame_range:
                ...
                if result['success']:
                    if result['is_best']:
                        self.logger.info(f"New Best Frame: {frame_number} With Width {result['width']:.2f}px")
                    else:
                        if result['has_measurement']:
                            self.logger.debug(f"Frame {frame_number}: Width Measured: {result['width']:.2f}px")
                        else:
                            self.logger.debug(f"Frame {frame_number}: No Valid Measurement")
                else:
            ...
            if result.returncode == 0:
                ...
                if lines:
                    try:
                        ...
                        self.logger.debug(f"State After Frame {frame_number}: frames_processed={self.state.frames_processed}, min_width={self.state.min_width_found}, has_best_info={self.state.best_frame_info is not None}")
                        if self.state.best_frame_info:
                            self.logger.debug(f"Best Frame Info: {self.state.best_frame_info}")
    ...
    def _output_result(self):
        ...
        if self.config.verbose:
            ...
            self.logger.info(f"Final state - min_width_found: {self.state.min_width_found}")
            self.logger.info(f"Final state - best_frame_info: {self.state.best_frame_info}")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Persistent Frame Processor For Tray Width Analysis')
